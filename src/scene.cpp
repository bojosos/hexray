...
bool DefaultSceneParser::parse(const char* filename, Scene* ss)
{
	...
	while (fgets(line, sizeof(line), f)) {
		...
		replaceRandomNumbers(curLine, line);
...
void Scene::visitSceneElements(std::function<void(SceneElement*)> visitor)
{
	for (auto& element: geometries) visitor(element);
	for (auto& element: textures) visitor(element);
...
void GlobalSettings::fillProperties(ParsedBlock& pb)
{
	...
	pb.getIntProp("foveatedRadius", &foveatedRadius, 0, 1000);
}
